...
// every other BYOK/integration read in this codebase is.
func NewGateway(appPool *pgxpool.Pool, encryptionKey []byte, registry *Registry) *Gateway {
	return &Gateway{appPool: appPool, encryptionKey: encryptionKey, registry: registry}
}
...
func (g *Gateway) ExecuteTool(ctx context.Context, orgID pgtype.UUID, service, toolName string, args map[string]any) (*gomcp.CallToolResult, error) {
